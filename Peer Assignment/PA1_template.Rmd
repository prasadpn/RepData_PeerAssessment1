...
library(lattice)
library(grid)
gp <- data.frame(aggregate(histStepAll$steps~histStepAll$interval+histStepAll$day,FUN = mean))
colnames(gp) <- c("interval","day","steps")
xyplot(gp$steps~gp$interval|gp$day,data = gp,type = ("l"),layout = c(1,2),ylab="Number of steps",xlab="Interval")
```
